...
OceanBase 不同版本之间的告警配置路径会稍微不一致（告警通道内容一致），本文由 4.2.2 版本为例，其他版本可以参考其官网文档寻找路径指引
...
#### 4. 配置通道配置中的脚本内容
请将脚本中的 token 参数补充 FlashDuty 推送地址中的 integration_key 值

```
#!/usr/bin/env bash

function sendToFlashDuty() {
    URL="${address}/event/push/alert/standard?integration_key=${token}"
...
#### 5. Response 校验信息填写 {} 即可

#### 6. 消息配置中的告警消息格式选择 Markdown

#### 7. 告警消息模板选择简体中文，并填写以下内容并提交
其他语言没有适配，如果需要使用其他语言可以修改脚本中的告警级别和状态相关的内容
```
OCP告警通知-单条告警
- 告警ID： ${alarm_id}
- 名称：${alarm_name}
...
#### 3. 推送消息配置
推送消息语言选择简体中文，告警通道选择FlashDuty，其他按需配置并提交

<img alt="drawing" width="600" src="https://fcpub-1301667576.cos.ap-nanjing.myqcloud.com/flashduty/doc/oceanbase-3.png" />
</div>

:::tip
- 建议开启恢复通知，这样可以自动恢复推送到FlashDuty的告警事件
- OceanBase 4.0.1 版本以上支持邮件推送告警，可以选择FlashDuty的邮件告警集成接收
- 通过邮件告警集成接收 OceanBase 的告警，需要自行提取标签内容
